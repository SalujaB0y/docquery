...
- **Works in Hindi too, with a measured threshold, not a guess.** Chunking, scoring, and embeddings are all language-agnostic, and `gpt-4o-mini` answers fluently in Hindi with correct citations out of the box. The one thing that doesn't transfer is `SIMILARITY_THRESHOLD`: 0.25 (tuned for English) caps Hindi retrieval hit rate at 78.6%, while 0.15 clears 100% with no cost to fallback accuracy — measured with a dedicated Hindi eval corpus (`npm run eval:hindi`, [hindi_corpus.txt](backend/src/eval/hindi_corpus.txt) / [hindi_pairs.json](backend/src/eval/hindi_pairs.json)), not assumed. Set `SIMILARITY_THRESHOLD=0.15` for a Hindi-only deployment.
