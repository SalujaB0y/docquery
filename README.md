...
language sql stable
as $$
  select id, content, 1 - (embedding <=> query_embedding) as similarity, chunk_index, token_count
  from chunks
...
- **Streaming responses** — the query endpoint currently returns one JSON blob after the full completion finishes. Switching to SSE and streaming tokens to the frontend would make the UI feel a lot faster.
- **Multi-document querying** — right now every chunk from every uploaded document is searched together. Adding a `document_id` filter to `match_chunks` and letting the frontend scope a query to one document (or all of them) is the natural next step.
